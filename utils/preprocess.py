...
def extract_embeddings(docs):
  ...
  for doc in docs:
    ...
    if len(doc) == 0:
      ...
      doc_embedding = np.mean([token.vector.get() for token in doc], axis=0)
      embeddings.append(doc_embedding)
  print(embeddings[0].shape if embeddings else 'empty')
  return embeddings

def preprocess_text_embeddings(data_folder, given_folder, language_model="en_core_web_trf"):
...
def preprocess_text_bow(data_folder, given_folder, language_model="en_core_web_sm", lemmatize=True):
  texts = read_texts(data_folder, given_folder)
  nlp_instance = get_nlp(language_model)
  processed_texts = []
